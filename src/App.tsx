    ...
    <div className="min-h-[100svh] lg:h-screen lg:w-screen bg-dark-bg text-white font-sans selection:bg-f1-red selection:text-white lg:overflow-hidden flex flex-col relative">
      ...
      <main className="flex flex-col-reverse lg:grid lg:grid-cols-[380px_1fr] flex-1 lg:overflow-hidden">
        ...
        <section className="flex-1 p-4 lg:p-6 flex flex-col bg-dark-bg lg:overflow-hidden relative z-10">
          <AnimatePresence mode="wait">
            {!selectedSession ? (
              <motion.div key="awaiting-input" initial={{ opacity: 0 }} animate={{ opacity: 0.2 }} exit={{ opacity: 0 }} className="flex-1 min-h-[60svh] lg:min-h-0 flex flex-col items-center justify-center text-center bg-dark-surface/20 p-8 rounded-xl border border-dark-border lg:bg-transparent lg:border-0">
              ...
              <motion.div key="telemetry-dashboard" initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.98 }} className="flex-1 flex flex-col min-h-[60svh] lg:min-h-0">
              <div className="mb-4 flex flex-col items-center text-center lg:flex-row lg:items-end lg:text-left justify-between gap-4">
                ...
                <div className="flex-1 min-h-[60svh] lg:min-h-0 border border-dark-border p-2 lg:p-4 bg-dark-surface/30 rounded-xl lg:rounded-lg relative overflow-hidden flex flex-col no-scrollbar">
                <div className="flex flex-nowrap items-center gap-1 mb-4 relative z-10 overflow-hidden w-full">
                  ...
                  {viewMode === 'single' && (
                     <div className="flex flex-nowrap gap-1 min-w-0 shrink">
                      {METRICS.map((m) => (
                        <button key={m} onClick={() => setSelectedMetric(m)} className={cn("px-1.5 sm:px-3 pb-1.5 pt-[7px] text-[7px] sm:text-[9px] font-mono font-bold uppercase tracking-widest transition-all border rounded-sm leading-none shrink min-w-0 truncate", selectedMetric === m ? "bg-f1-red/20 text-f1-red border-f1-red" : "bg-dark-bg/50 text-white/40 border-dark-border hover:border-white/20 hover:text-white/60")}>{m}</button>
                      ))}
                    </div>
                  )}
                </div>
                ...
                <AnimatePresence mode="wait">
                  {!isAnyLoading && telemetryData.length > 0 && (
                    <motion.div key={`${viewMode}-${selectedMetric}-${JSON.stringify(selectedLaps)}-${JSON.stringify(selectedDrivers)}`} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex-1 flex flex-col min-h-[40svh] lg:min-h-0">
                      {viewMode === 'single' ? (
                        ...
                        <div className="flex-1 flex flex-col min-h-0">
                          {METRICS.map((m, idx) => (
                            <div key={m} className="flex flex-col flex-1 min-h-[250px] shrink-0 lg:min-h-0 lg:shrink border-b border-white/5 last:border-0 pt-2">
              ...
              <motion.div key="loading-telemetry" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex-1 min-h-[60svh] lg:min-h-0 flex items-center justify-center">
      ...
      {isExporting && (
        <div className="fixed left-[-9999px] top-[-9999px]">
          <div ref={exportRef} style={{ width: '1920px', height: '1080px' }} className="bg-dark-bg p-10 flex flex-col relative overflow-hidden">
            <div className="absolute inset-0 carbon-pattern opacity-10 pointer-events-none" />

            <div className="flex justify-between items-start mb-6 relative z-10">

              <div className="flex flex-col gap-5">
                <img src="/assets/uploads/lap_logo.png" alt="Lap-Check Logo" crossOrigin="anonymous" className="h-10 w-auto object-contain object-left" />
                <div>
                  <h2 className="text-5xl font-black uppercase tracking-tighter leading-none mb-1.5">
                    {selectedMeeting?.meeting_name}
                  </h2>
                  <p className="text-xl font-mono opacity-50 uppercase tracking-widest">
                ...
                {selectedDrivers.map((num, idx) => {
                  ...
                  return (
                    <div
                      ...
                      className="flex items-stretch gap-4 bg-dark-surface/80 border border-dark-border p-3 rounded-md min-w-[215px] shadow-xl"
                    >
                      <div
                        className="w-2 rounded-full shrink-0"
                        style={{
                          background: isDashed
                            ? `repeating-linear-gradient(to bottom, #${d?.TeamColor || '888'}, #${d?.TeamColor || '888'} 10px, transparent 10px, transparent 20px)`
                            : `#${d?.TeamColor || '888'}`
                        }}
                      />

                      <div className="flex flex-col justify-between flex-1 gap-1.5 min-w-0">
                        <div className="flex items-center justify-between gap-4">
                          <div className="flex items-center gap-2.5 shrink-0">
                            <span className="font-black text-3xl tracking-tighter leading-none">{d?.Abbreviation}</span>
                          </div>

                          {lap && (
                            <div className="shrink-0 flex items-center justify-center">
                              {/* Größeres TyreIcon für den 1920x1080 Export */}
                        ...
                        {lap && (
                          <div className="flex flex-col mt-1.5 min-w-0">
                            ...
                            <span className="text-xl font-mono text-f1-red font-bold leading-none mt-1.5 truncate">
                              {formatLapTime(lap.LapTime)}
                            </span>
                          </div>
                        )}
            ...
            <div className="flex-1 border border-dark-border bg-dark-surface/40 rounded-2xl relative overflow-hidden p-5 shadow-2xl flex flex-col no-scrollbar" style={{ minHeight: 0, minWidth: 0, position: 'relative' }}>
              {viewMode === 'single' ? (
                <div className="flex-1 flex flex-col min-h-0">
                  <TelemetryChart
                    data={telemetryData}
                ...
                <div className="flex-1 flex flex-col min-h-0">
                  {METRICS.map((m, idx) => (
                    <div key={m} className="flex flex-col flex-1 shrink-0 lg:shrink border-b border-white/5 last:border-0 pt-2">
                      <TelemetryChart
                        data={telemetryData}
                        metric={m}
                        results={results}
            ...
            <div className="mt-8 flex justify-end">
               <span className="text-sm font-mono opacity-30 uppercase tracking-[0.3em]">
                 Generated by LapCheck // Telemetry: {viewMode === 'single' ? selectedMetric.toUpperCase() : 'ALL METRICS'}
               </span>
            </div>
          </div>
        </div>
